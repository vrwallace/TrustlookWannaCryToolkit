...
def scan(host):
    ...
    try:
        ...
        if status == 0xc0000205:
            ...
            if double_infection:
                #print('%s\t%s\tsystem is vulnerable, DoublePulsa infection - Arch: %s Key:0x%x ' % \
                 #     (host,hostname, get_arch(sig2),xor_key(sig1)))
                rtnmessage = host + '\t' + hostname + '\t' + 'system is vulnerable, DoublePulsa infection - Arch: ' + get_arch(sig2) + 'Key:0x' + xor_key(sig1) 
            else:
                #print('%s\t%s\tsystem is vulnerable' % (host,hostname))
                rtnmessage = host + '\t' + hostname + '\t' + 'system is vulnerable'
        elif status == 0xc0000008 or status == 0xc0000022:
            # STATUS_INVALID_HANDLE or STATUS_ACCESS_DENIED
            #print('%s\t%s\tsystem is not vulnerable' % (host,hostname))
            rtnmessage = host + '\t' + hostname + '\t' + 'system is not vulnerable'  
        else:
            #print('%s\ts%\tcan not detect vulnerable status' % (host,hostname))
            rtnmessage = host + '\t' + hostname + '\t' + 'can not detect vulnerable status'
...
def main():
    ...
    if '/' in scan_network:
        ...
        pool = multiprocessing.Pool(processes=254)
        resultst=pool.map(scan, hosts)
        
        
        for j  in range(len(resultst)):
            if  resultst[j].strip() <> "" : print(resultst[j])
